...
class FormController extends Controller
{
    ...
    {
        $postcode = $request->check_postcode;
        $postcodes = PincodeModel::where('pincode','=',$postcode)->first();
      
        if($postcodes === NULL){
            return view('form_contact',compact('postcode'));
        }
        else{
            return view('form',compact('postcode'));
        }
    ...
    public function sendEmail(Request $request)
    {
     $this->validate($request, [
      ...
      Mail::to('mubimusthafa51@gmail.com')->send(new SendMail($data));
    ...
    public function BroadbandformSubmit(Request $request)
    {
       ...
       // dd($orderTotal);
      //  $order_details[] = $data['all_order_details'];

      //  $oder_total = array_push($order_details,$data['order_total']);
      //  dd($order_details);
        ...
        /*{"current_service_provider":"","number_wish_to_transfer":"","address_wish_to_transfer":"","preferred_transfer_date":"","asap":0,"transfer_second_number":0}*/
         ...
         $mobilePortNo = $data['port_no'] == "yes" ? 1 : 0;
         ...
         //dd($router_delivery_address);
         /*$mobileNoPortDetails = [
             'cur_mobile_number' => $data['cur_mobile_number'] ? $data['cur_mobile_number'] : "",
             'cur_pac_code' => $data['cur_pac_code'] ? $data['cur_pac_code'] : ""
         ];*/
         /*{"cur_mobile_number":"","cur_pac_code":""}*/
         $mobileNoPortDetails=[];
         $mobileNoPortDetails['cur_mobile_number']=isset($data['cur_mobile_number'])?$data['cur_mobile_number']:'';
         $mobileNoPortDetails['cur_pac_code']=isset($data['cur_pac_code'])?$data['cur_pac_code']:'';
       ...
       //  dd(implode ("| ", $routerExtra));
        ...
        /* $billingAddress = [
         ...
         $order = Order::create([
             ...
             'mobile_package' => (isset($data['mobile_plan']) ) ? $data['mobile_plan'] : "",
             ...
             'port_mobile_no' => $mobilePortNo,
             'mobile_number_port_details' => (!empty($mobileNoPortDetails))?$mobileNoPortDetails:null,
         ...
         /* $plan_type='';
         if(isset($data['broadband_plan'])){
          $plan_type='broadband';   
         }else if(isset($data['voice_package'])){
              $plan_type='voice';
         }else if(isset($data['mobile_plan'])){
               $plan_type='mobile';
         }
         dd($plan_type);*/
         ...
         $user_plan_logs=UserPlanLog::create([
             'user_id' => $user->id,
             'customer_id'=>$customer->id,
             'plan_name'=>(isset($data['mobile_plan']) ) ? $data['mobile_plan']:'None',
             'plan_type'=>(isset($data['mobile_plan']) ) ? 'mobile':null,
             'added_from'=>'website',
             'added_by'=>$user->id,
             'activation_date'=>(isset($data['acvdate']) ) ? $data['acvdate'] : ""
             ]);
